            ...
            if (orders && orders.length > 0) { //if start
                orders.forEach(ord => { //forEach start
                    html += `
                        ...
                        <th> ${ord.coupname} </th>
                        <th> <button type="button" class="cancelButton" onclick="returnCheck(${ord.orddetailcode}), invAutoUpdateReturn(${ord.orddetailcode})"> 취소확정 </button> </th>
                        </tr>`
                }); //forEach end
            }; //if end
            //목록 출력
...
//취소 확정 함수
function returnCheck(orddetailcode) {
...
function invAutoUpdateReturn(orddetailcode) {
    $.ajax({
        async: false,
        method: "get",
        url: "/inventory/autoreturn",
        data: { orddetailcode: orddetailcode },
