...
getProductYearlyTable()

// 연단위 매출 조회 (레코드 단위 : 월)
function getProductYearlyTable(){
    tableHTML = ""  // 테이블 HTML
    $.ajax({  // 테이블 데이터 가져오는 ajax
        async : false,
        method : "GET",
        url : "/sales/product/yearlytable",
        ...
        success : r => {
            ...
            // 테이블 HTML 생성 루프
            r.forEach(dto => {                
                chartLabel.push(dto.prodcode)
                revenueChartData.push(dto.revenue)
                saleAmountChartData.push(dto.saleAmount)
                incomeChartData.push(dto.income)
                tableHTML += `<td>${dto.prodcode}</td><td>${dto.prodname}</td><td>${dto.prodprice}</td><td>${dto.prodcatename}</td><td>${dto.ordered}</td><td>${dto.returned}</td><td>${dto.canceled}</td><td>${dto.completed}</td><td>${currencyFormat(dto.revenue)}</td><td>${currencyFormat(dto.saleAmount)}</td><td>${currencyFormat(dto.income)}</td></tr>`
            })
            // 완성된 HTML을 삽입
