...
function getCouponTable(){
    ...
    $.ajax({  // 테이블 데이터 가져오는 ajax
        ...
        success : r => {
            ...
            // 테이블 HTML 생성 루프
            r.forEach(dto => {                
                chartLabel.push(dto.coupname)
                revenueChartData.push(dto.revenue)
                saleAmountChartData.push(dto.saleAmount)
                incomeChartData.push(dto.income)
                tableHTML += `<td>${dto.coupcode}</td><td>${dto.coupname}</td><td>${dto.coupsalerate}</td><td>${dto.coupexpdate}</td><td>${dto.orders}</td><td>${dto.ordered}</td><td>${dto.returned}</td><td>${dto.canceled}</td><td>${dto.completed}</td><td>${currencyFormat(dto.revenue)}</td><td>${currencyFormat(dto.saleAmount)}</td><td>${currencyFormat(dto.income)}</td></tr>`
            })
